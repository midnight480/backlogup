        ...
        return (
            <div className={`mt-xs ${level > 0 ? "ml-xl border-l border-outline-variant" : ""}`}>
                ...
                >
                    ...
                    <Link to={`/wikis/${node.wiki.id}`} className="material-symbols-outlined text-sm hover:text-primary px-1" onClick={e => e.stopPropagation()}>description</Link>
                </div>
                {open && (
                    <div className="ml-xl mt-xs border-l border-outline-variant">
                        {childNodes.map(child => <WikiTreeNodeView key={child.label} node={child} currentWikiId={currentWikiId} level={level + 1} />)}
                    </div>
                )}
            </div>
        );
                    ...
                    <div className="p-sm">
                        {wikiStore.loadingList ? (
                            ...
                            rootChildren.map((node) => (
                                <WikiTreeNodeView key={node.label} node={node} currentWikiId={wikiId} />
                            ))
                        ) : (
                ...
                <div className="max-w-4xl mx-auto">
                    {(!wikiId || wikiId === 'home') ? (
                        <div className="text-center mt-20">
                            ...
                            <p className="text-body-md text-on-surface-variant mt-2">Select a page from the directory on the left to view its contents.</p>
                        </div>
                        ...
                        <>
                            ...
                            <nav className="flex items-center gap-2 text-on-surface-variant font-label-md text-label-md uppercase mb-lg">
                                <span className="hover:text-primary transition-colors cursor-pointer" onClick={() => navigate('/wikis/home')}>WIKI</span>
                            ...
                            <div className="flex justify-between items-start mb-xl pb-6 border-b border-outline-variant">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 text-on-surface-variant text-body-sm">
                                        ...
                                        <span className="flex items-center gap-1">
                                            <img src={`/assets/users/${wikiStore.wiki.createdUser?.id}/icon`} alt="icon" className="w-4 h-4 rounded-full" onError={(e) => (e.currentTarget.style.display = 'none')} />
                                    ...
                                    {wikiStore.wiki.tags && wikiStore.wiki.tags.length > 0 && (
                                        <div className="flex gap-2 mt-3">
                                            {wikiStore.wiki.tags.map((tag) => (
                                                <span key={tag.id} className="px-2 py-0.5 bg-surface-container-high rounded text-[11px] font-bold text-on-surface-variant">
                                                    {tag.name}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                {isMarkdown ? (
                                    <ReactMarkdown
                                        ...
                                        components={{
                                            ...
                                            h1: ({children}) => <h1 className="font-headline-lg text-headline-lg mb-4 border-b border-outline-variant pb-2">{children}</h1>,
                                            h2: ({children}) => <h2 className="font-headline-md text-headline-md mt-8 mb-2 border-b border-outline-variant pb-2">{children}</h2>,
                                            ...
                                            blockquote: ({children}) => <blockquote className="border-l-4 border-primary bg-surface-container-low px-4 py-2 mb-4 italic rounded-r">{children}</blockquote>,
                            ...
                            {wikiStore.wiki.attachments && wikiStore.wiki.attachments.length > 0 && (
                                <div className="mt-12 p-6 border border-outline-variant bg-surface-container-lowest rounded-xl">
                                    <h4 className="font-headline-sm text-headline-sm mb-4 text-on-surface">Attachments ({wikiStore.wiki.attachments.length})</h4>
                                        ...
                                        {wikiStore.wiki.attachments.map((attachment) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    <div className="text-body-md font-medium text-on-surface truncate group-hover:text-primary">{attachment.name}</div>
                                                    <div className="text-[11px] text-tertiary">{(attachment.size / 1024).toFixed(1)} KB</div>
                                                </div>
                                                <span className="material-symbols-outlined text-outline group-hover:text-primary opacity-0 group-hover:opacity-100 transition-opacity">download</span>
                                            </a>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="text-center mt-20 text-tertiary">
                            Wiki page not found or unable to load.
                        </div>
                    )}
                </div>
            </section>
