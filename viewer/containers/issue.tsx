      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <div className={`flex flex-col ${issue.dueDate && dayjs(issue.dueDate).isBefore(dayjs()) ? "text-red-600" : ""}`}>
            <span
              className={`text-xs font-medium uppercase tracking-wider ${issue.dueDate && dayjs(issue.dueDate).isBefore(dayjs()) ? "text-red-500" : "text-gray-400"}`}
            >
              期限日
            </span>
            <span
              className={`font-medium mt-0.5 ${issue.dueDate && dayjs(issue.dueDate).isBefore(dayjs()) ? "text-red-600" : "text-gray-900"}`}
            >
              {issue.dueDate ? dayjs(issue.dueDate).format("YYYY/MM/DD") : "-"}{" "}
              {issue.dueDate && dayjs(issue.dueDate).isBefore(dayjs()) ? "🔥" : ""}
            </span>
          </div>
        ...
        <div className="p-6">
          <UserHeader user={issue.createdUser}>
            <div className="text-xs text-gray-500 mt-0.5">
              登録日: {issue.created ? dayjs(issue.created).format("YYYY/MM/DD HH:mm:ss") : ""}
            </div>
          </UserHeader>
        ...
        <div className="space-y-6">
          {issueStore.comments
            ...
            .map((comment) => (
              <div
                key={comment.id}
                className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 md:p-6 transition-all hover:shadow-md"
              >
                ...
                <div className="mt-4 md:ml-12 pl-1 border-l-2 border-transparent">
                        ...
                        switch (changeLog.field) {
                          case "notification":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                お知らせ:{" "}
                                <span className="font-medium text-gray-900">{notificationType(changeLog.notificationInfo.type)}</span>
                              </div>
                            );
                          case "limitDate":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                期限日: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue}</span>
                              </div>
                            );
                          case "assigner":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                担当者: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue}</span>
                              </div>
                            );
                          case "parentIssue":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                親課題: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue}</span>
                              </div>
                            );
                          case "description":
                              ...
                              <div className="flex items-start gap-2 mt-2" key={index}>
                                ...
                                <details className="group flex-1 border border-gray-200 rounded-lg bg-white shadow-sm overflow-hidden">
                                  ...
                                  <div className="border-t border-gray-200 p-3 text-sm text-gray-600 bg-gray-50/50">
                                    <div className="whitespace-pre-wrap bg-red-50/50 text-red-900 p-2 rounded border border-red-100">
                                      {changeLog.originalValue}
                                    </div>
                                    <div className="my-2 text-center text-gray-400 flex justify-center">
                                      <ArrowDownIcon />
                                    </div>
                                    <div className="whitespace-pre-wrap bg-green-50/50 text-green-900 p-2 rounded border border-green-100">
                                      {changeLog.newValue}
                                    </div>
                                  </div>
                                </details>
                          ...
                          case "component":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                カテゴリー: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "resolution":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                完了理由: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "milestone":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                マイルストーン: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "priority":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                優先度: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "issueType":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                種別: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "estimatedHours":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                予定時間: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "actualHours":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                実績時間: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "status":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                状態: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          case "attachment":
                          ...
                          case "summary":
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                件名: {changeLog.originalValue || "未設定"} <span className="mx-1 text-gray-400">➡️</span>{" "}
                                <span className="font-medium text-gray-900">{changeLog.newValue || "未設定"}</span>
                              </div>
                            );
                          default:
                            return (
                              <div className={logClass} key={index}>
                                <Bullet />
                                不明イベント: <span className="font-mono text-xs bg-gray-100 p-1 rounded">{JSON.stringify(changeLog)}</span>
                              </div>
                            );
                        }
                      })}
                  ...
                  {comment.created !== comment.updated && <div className="mt-2 text-xs text-gray-400 italic">（編集済み）</div>}
                </div>
